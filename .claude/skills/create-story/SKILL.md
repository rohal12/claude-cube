...
## Phase 1: Story Concept (Interactive)

Invoke the `story-architect` agent:

```
Use the story-architect agent to work with the user to develop the story concept.
The user's story idea is: [include the user's prompt here]
Write the Story Bible to story-workspace/story-bible.md when done.
```
...
2. Re-invoke `sugarcube-expert` with the specific error:
   ```
   The build failed with this error:
   [paste error]
   Fix the issue in the affected .twee file(s).
   ```
3. Re-run `npm run build`
4. Repeat up to 3 times. If still failing after 3 attempts, proceed to Checkpoint 2 with the errors.

Update state.

## Phase 7: Static Review
...
## Phase 8: Playwright Playthrough Testing
